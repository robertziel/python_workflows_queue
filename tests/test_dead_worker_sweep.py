...
def test_flags_stale_worker_holding_running_job():
    ...
    _running_job_owned_by("spark", model="wan_i2v")
    _put_heartbeat("spark", "gpu", last_seen_age_s=120)  # 29-min-style staleness

    flagged = node_queue.flag_stale_workers_holding_running_jobs(stale_after_s=30)

    assert len(flagged) == 1
    row = flagged[0]
    assert row["host_label"] == "spark"
    assert row["queue"] == "gpu"
    assert row["running_jobs"] == 1
    # The durable, queryable marker is stamped.
    assert _flagged_at("spark", "gpu") is not None
...
def test_fresh_heartbeat_holding_running_job_is_not_flagged():
    ...
    NOT be flagged."""
    _running_job_owned_by("spark", model="wan_i2v")
    _put_heartbeat("spark", "gpu", last_seen_age_s=2)  # fresh

    flagged = node_queue.flag_stale_workers_holding_running_jobs(stale_after_s=30)

    assert flagged == []
    assert _flagged_at("spark", "gpu") is None
...
def test_attribution_is_scoped_to_the_jobs_queue_family():
    """A host can run SEVERAL workers under one ``host_label`` (beelink runs a
    cpu AND a gpu worker). A gpu job (claimed_by=beelink, queue=gpu) must be
    attributed to the beelink/gpu heartbeat, NOT the beelink/cpu one — so a
    ...
    _running_job_owned_by("beelink", queue="gpu", model="wan_i2v")
    _put_heartbeat("beelink", "gpu", last_seen_age_s=120)  # gpu worker wedged
    _put_heartbeat("beelink", "cpu", last_seen_age_s=2)    # cpu worker healthy

    flagged = node_queue.flag_stale_workers_holding_running_jobs(stale_after_s=30)

    assert len(flagged) == 1
    assert (flagged[0]["host_label"], flagged[0]["queue"]) == ("beelink", "gpu")
    assert _flagged_at("beelink", "cpu") is None  # healthy sibling untouched
...
def test_stale_sibling_without_its_own_queues_job_is_not_flagged():
    ...
    _running_job_owned_by("beelink", queue="gpu", model="wan_i2v")
    _put_heartbeat("beelink", "gpu", last_seen_age_s=2)    # gpu worker healthy
    _put_heartbeat("beelink", "cpu", last_seen_age_s=300)  # cpu worker stale but idle
...
def test_flag_is_idempotent_within_window():
    ...
    _running_job_owned_by("spark", model="wan_i2v")
    _put_heartbeat("spark", "gpu", last_seen_age_s=120)
...
def test_heartbeat_refresh_clears_the_flag():
    ...
    node_queue.upsert_worker_heartbeat(
        host_label="spark", queue="gpu", concurrency=1,
    )
    assert _flagged_at("spark", "gpu") is None
...
def test_re_flags_after_recovery_then_stale_again():
    ...
    job = _running_job_owned_by("spark", model="wan_i2v")
    _put_heartbeat("spark", "gpu", last_seen_age_s=120)
    ...
    node_queue.upsert_worker_heartbeat(host_label="spark", queue="gpu", concurrency=1)
    assert _flagged_at("spark", "gpu") is None
...
def test_default_threshold_used_when_arg_omitted(monkeypatch):
    ...
    monkeypatch.delenv("AI_LEADS_STALE_WORKER_AFTER_S", raising=False)
    _running_job_owned_by("spark", model="wan_i2v")
    _put_heartbeat("spark", "gpu", last_seen_age_s=40)
...
def test_sweep_logs_actionable_dead_worker_line(caplog):
    ...
    assert any("DEAD WORKER" in m and "spark" in m and "wedged" in m for m in msgs), msgs
...
def test_sweep_does_not_touch_the_job_row():
    ...
    job_id = _running_job_owned_by("spark", model="wan_i2v")
    _put_heartbeat("spark", "gpu", last_seen_age_s=120)
    ...
    assert row["claimed_by"] == "spark"      # not cleared
